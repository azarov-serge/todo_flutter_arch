...
part 'task_update_bloc.freezed.dart';
...
class TaskUpdateBloc extends Bloc<TaskUpdateEvent, TaskUpdateState> {
  ...
  factory TaskUpdateBloc.create() => TaskUpdateBloc(
        store: getIt.get(),
      );

  TaskUpdateBloc({required this.store}) : super(const TaskUpdateState()) {
    on<_ChangeStateEvent>(_changeState);

    on<_UpdateEvent>(_update);
    ...
    _storeListener = store.onChange.listen((appState) {
      add(TaskUpdateEvent.changeState(appState: appState));
    });

    final request = taskUpdateRequest.copyWith(id: _id);
  ...
  void _update(_UpdateEvent event, emit) {
    _id = event.task.id;
